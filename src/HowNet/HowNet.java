...
public class HowNet {
    ...
    /**
     * 加载 glossay.dat 文件
     * glossory.dat文件每一行格式：     汉语词语     词性      词语义原集合
     */
    public void loadGlossary() {
        String line = null;
        BufferedReader reader = null;
        try {
            reader = new BufferedReader(new InputStreamReader(new FileInputStream("HowNetDict/glossary.dat"), "gb2312"));   // 读入gb2312格式文本
    ...
    /**
     * 加载义原文件 WHOLE.DAT
     * glossory.dat文件每一行格式：     义原ID        义原(英文|中文)表示       父义原ID
     */
    public void loadWhole(){
        String line = null;
        BufferedReader reader = null;
        try {
            reader = new BufferedReader(new InputStreamReader(new FileInputStream("HowNetDict/WHOLE.dat"), "gb2312"));   // 读入gb2312格式文本
            line = reader.readLine();
            while (line != null) {
                line = line.trim().replaceAll("\\s+", " ");
                String[] strs = line.split(" ");
    ...
    private double innerSimWord(String word1, String word2) {
        ...
        boolean isPrimitive1 = primitive.isPrimitive(word1, PRIMITIVESID_en, PRIMITIVESID_zh);
        boolean isPrimitive2 = primitive.isPrimitive(word2, PRIMITIVESID_en, PRIMITIVESID_zh);
        // 两个义原
    ...
    public int disPrimitive(String primitive1, String primitive2) {
        Primitive primitive = new Primitive();
        List<Integer> list1 = primitive.getParents(primitive1, ALLPRIMITIVES_zh, PRIMITIVESID_zh, PRIMITIVESID_en);
        List<Integer> list2 = primitive.getParents(primitive2, ALLPRIMITIVES_zh, PRIMITIVESID_zh, PRIMITIVESID_en);
        for (int i = 0; i < list1.size(); i++) {
            int id1 = list1.get(i);
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("请输入两个待比较相似度的中文词，以空白符分割");
        while ( sc.hasNext() ){
...
//        System.out.println("请输入两个待比较相似度的中文词（包括对应的词性， 中国/N)，以空白符分割");
//        while ( sc.hasNext() ){
//            String line = sc.nextLine();
//            String[] wWords = line.split("\\s");
//
//            Word wWord1 = new Word(wWords[0].split("/")[0], wWords[0].split("/")[1]);
//            Word wWord2 = new Word(wWords[1].split("/")[0], wWords[1].split("/")[1]);
//
//            System.out.format("simWord(Word, Word): sim(%s, %s)=%f\n", wWord1.getWord(), wWord2.getWord(), howNet.simWord(wWord1, wWord2));
//        }


//        System.out.println("请输入两个待比较相似度的义原，以空白符分割");
//        while ( sc.hasNext() ){
//            String line = sc.nextLine();
//            String[] primitives = line.split("\\s");
//            System.out.format("disPrimitive(%s, %s)=%d\n", primitives[0], primitives[1], howNet.disPrimitive(primitives[0], primitives[1]));
//        }
